...
**Versi Design**: 2.6.0 (Google Stitch AI UI System & Dual Firebase Architecture)  
...
Antarmuka Pelanggan **Nefakky** mengadopsi standar **"Google Stitch Artisanal Luxury"**. Desain ini dirancang khusus untuk menciptakan pengalaman pemesanan makanan premium khas Nusantara yang hangat, responsif, dan estetik, dengan transisi yang mulus dari katalog hingga pelacakan *real-time*.

### Prinsip Utama UI/UX:
1. **Google Stitch Color System**: Kombinasi warna tanah (*earthy tones*) eksklusif: `#25160E` Deep Espresso, `#3C2A21` Espresso Container, `#934B19` Terracotta Accent, `#FBF9F5` Warm Cream Canvas.
...
4. **Alamat Produksi Resmi**: Seluruh item menampilkan lokasi produksi resmi terpusat: *Puri Bojong Lestari AF No 41, Rt 10 Rw 14, Kel. Pabuaran, Kec. Bojong Gede, Kabupaten Bogor, Provinsi Jawa Barat, Indonesia*.
5. **Alur Checkout 4-Tahap & Payment Gateway Engine**: Visualisasi stepper transaksi yang intuitif (`1. Keranjang` $\rightarrow$ `2. Pengiriman` $\rightarrow$ `3. Pembayaran Midtrans` $\rightarrow$ `4. Selesai`) terintegrasi langsung dengan Midtrans Snap API & Sandbox Payment Simulator.
...
## 🖥️ 4. Spesifikasi Halaman & Komponen Pelanggan (Halaman Selesai)
...
* **Menu Links**:
  - `Beranda` (`/`)
  - `Katalog Menu` (`/menu`)
  - `Ulasan Rasa` (`/comments`)
  - `Pemesanan & Keranjang` (`/cart`)
* **Badges & Quick Action**:
  - Pill Badge Count `Keranjang` dengan nomor angka item aktif.
  - Tombol Quick Access `Status Pesanan` (`/notifications`) dengan indikator titik pulsa hijau/oranye real-time.
  - Avatar Pengguna yang mengarahkan langsung ke Halaman Akun & Profil (`/profile`).
...
* **Pill Category Filter**:
  - `Semua Menu`
  - `Makanan Berat` (Ayam Bakar Keraton, Gudeg Wijaya, Garang Asam, Krecek)
  - `Minuman` (Jus Mangga, Sirsak, Jambu)
  - `Menu Hemat` (Kombo Hemat)
* **Spesifikasi Product Card Component**:
  ...
  - Teks Alamat Produksi Resmi: *Puri Bojong Lestari AF No 41, Rt 10 Rw 14, Kel. Pabuaran, Kec. Bojong Gede, Kabupaten Bogor, Provinsi Jawa Barat, Indonesia*.
...
* **Pilihan 3-Varian Rasa Jus Minuman (1 Halaman)**:
  ...
  - Foto display utama dan thumbnail strip otomatis berganti warna dan gambar sesuai varian rasa yang dipilih.
...
* **Horizontal Stepper Component**:
  - `1. Keranjang` $\rightarrow$ `2. Pengiriman & Alamat` $\rightarrow$ `3. Pembayaran Midtrans` $\rightarrow$ `4. Selesai`.
...
* **Integrasi Gateway Pembayaran Midtrans Snap**:
  - Tombol **Bayar Sekarang** menghubungkan langsung ke backend `/api/midtrans/token`.
  - Mendukung BCA Virtual Account, QRIS, GoPay, dan Kartu Kredit Sandbox.
  - Modal instruksi lengkap dengan tombol tautan langsung ke **Midtrans Payment Simulator Sandbox** (`https://simulator.sandbox.midtrans.com/bca/va/index`).
...
* **Tab Edit Profil**: Form nama lengkap, nomor telepon WhatsApp, alamat pengiriman utama, dan foto avatar Google SSO.
...
* **Form Tambah Ulasan**: Modal pemberian ulasan baru dengan star-rating picker 1-5 bintang.
...
* **Multi-Option Login**: Tombol Google Sign-In Single-Sign-On (SSO) & Form Email/Password dengan pesan error Bahasa Indonesia.
